...
def remove_segments_with_ffmpeg(input_video, segments_file, output_video):
    import shutil
    ...
    # 构建未删除的时间段
    last_end_time = "00:00:00.000"  # 视频起始时间
    segments = []
    ...
    total_duration = f"{hours:02}:{minutes:02}:{seconds:06.3f}"

    # 添加未删除的片段
    for start, end in remove_intervals:
        if start != last_end_time:
            segments.append((last_end_time, start))
        last_end_time = end

    # 添加最后一个未删除的片段
    if last_end_time < total_duration:
        segments.append((last_end_time, total_duration))
...
if __name__ == "__main__":
    # 输入文件路径
    input_video = 'input/HU_6.mp4'
    vtt_file = "vtt/HU_6.vtt"  # 替换为你的 .vtt 文件路径
    ...
    output_video = "output/edited_video.mp4"
    ...
    remove_segments_with_ffmpeg(input_video, output_segment_file, output_video)
